...
// ========= Templates / render =========
// NOTE: price element is OUTSIDE .meta so absolute positioning targets .card
function cardTemplate(p, i) {
  const imgPart = p.img
    ? `<div class="thumb-wrap"><img class="thumb" src="${escapeAttr(p.img)}" alt="${escapeAttr(p.name || "Perfume")} bottle" loading="lazy" decoding="async" /></div>`
    : `<div class="thumb-wrap"><div class="thumb placeholder"></div></div>`;

  const notes = p.notes ? `<div class="notes">${escapeHTML(p.notes)}</div>` : "";

  // Always show a GBP tag; fallback "£—" if missing
  const priceText = (p.price !== undefined && p.price !== null && String(p.price).trim() !== "")
    ...
    <article class="card" style="--i:${i}" data-id="${escapeAttr(p.id || "")}">
      <div class="controls">
        <div class="icon-btn" title="Edit" data-edit="${escapeAttr(p.id || "")}">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM21.41 6.34a1.25 1.25 0 000-1.77l-2.34-2.34a1.25 1.25 0 00-1.77 0l-1.83 1.83 3.75 3.75 1.19-1.47z"/></svg>
        </div>
...
addForm?.addEventListener("submit", async (e) => {
  ...
  if (!payload.name || !payload.brand) { alert("Name and Brand are required"); return; }
...
function bindZoomHandlers() {
  ...
  const pad = 16; const W = 280, H = 360;
  function place(x, y) {
...
document.addEventListener('click', (e) => {
  ...
  else if (!e.target.closest('.menu')) menu.classList.remove('open');
});
